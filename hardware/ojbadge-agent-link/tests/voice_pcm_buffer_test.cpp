...
int main() {
    ...
    size_t written = 0;
    // Ten seconds of 32 KB/s PCM, while BLE drains only 16 KB/s.
    for (int ms = 0; ms < 10000; ms += 10) {
        ...
        if (ms % 20 == 0) {
            old_queue.Pop(block, 320);
            const size_t bytes = queue.Pop(block, 320);
            output.insert(output.end(), block, block + bytes);
        }
    }
    assert(old_overflow && written == 320000);
    while (queue.Size()) {
    ...
    std::puts("PASS: old 96KB overflow reproduced; full 10s PCM buffer under backpressure; 30s bound, wipe, wrap, MTU");
}
