...
void SendManifest(bool force) {
    ...
    if (s_cfg.transport == AGENT_TRANSPORT_WIFI) {
        ...
        budget = agentlink::ManifestChunkBudget(mtu);
    }
    if (budget == 0) {
        ESP_LOGW(TAG, "manifest deferred: no usable ATT MTU");
        return;
    }
    if (budget > 480) budget = 480;

    if (json.size() > budget * 256) {
        ESP_LOGW(TAG, "manifest exceeds 256 chunks; negotiate a larger MTU");
        return;
    }
...
void OnCtrlFrame(const uint8_t* data, size_t len) {
    ...
    if (f.command_id == 0x03) {
        ...
        if (f.payload.empty() && (s_bat_pct < 0 || esp_timer_get_time() - s_bat_observed_us > 15000000)) {
            status = 1; error = 1005;  // NoData; unknown battery is not 0%.
...
esp_err_t agent_link_report_battery(uint8_t percent, bool charging) {
    ...
    s_bat_observed_us = esp_timer_get_time();
