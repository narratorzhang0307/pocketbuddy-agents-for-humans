...
// ATT notification overhead (3), frame header (6), manifest chunk header (2).
// A default MTU of 23 leaves 12 bytes, not a 150-byte fallback.
constexpr size_t ManifestChunkBudget(uint16_t mtu) {
    const size_t overhead = 3 + kHeaderSize + 2;
    return mtu <= overhead ? 0 : (mtu - overhead > 480 ? 480 : mtu - overhead);
}
