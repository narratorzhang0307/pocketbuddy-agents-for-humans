...
import { readVideoFrame } from "../video/videoFrames";
...
export default function VideoAnalysisScreen({ navigation, route }: Props) {
  ...
  const select = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!selected) return;
    stop(); setFile(null); setDuration(0); setConsent(false); setProgress(emptyVideoProgress()); setError(""); setStatus("ready");
  ...
  const start = async () => {
    ...
    setStatus("running"); setError(""); setProgress(emptyVideoProgress());
    ...
    try {
      ...
      await analyzeVideo({ duration, exercise, consent, signal: controller.signal, provider,
        frameAt: (time, signal) => readVideoFrame(video, canvas, time, signal),
        onProgress: next => {
          if (!current()) return;
          setProgress(next);
  ...
  return <section aria-label="预录视频分析" style={{ flex: 1, minHeight: 0, overflowY: "auto", boxSizing: "border-box", padding: 16, background: "#f2f3ed", color: "#171717", fontFamily: "system-ui, sans-serif" }}>
    ...
    {file && <div style={{ ...card, marginTop: 12 }}>
      ...
      <p style={{ fontSize: 12, color: "#50554c", lineHeight: 1.65, marginBottom: 0 }}>每秒取 4 帧，依次交给真实模型；分析可能比视频时长更久。请保持页面前台。教练提示沿用当前声音通道。</p>
    </div>}
