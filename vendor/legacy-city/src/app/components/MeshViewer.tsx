...
// 用户主力路径：自有 GPU、Scaniverse、Polycam 等把展品扫成 .glb 带回家 → 导入这里展示。
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const { GLTFLoader } = await importWithChunkRecovery(() => import('three/examples/jsm/loaders/GLTFLoader.js'));
        const { OrbitControls } = await importWithChunkRecovery(() => import('three/examples/jsm/controls/OrbitControls.js'));
