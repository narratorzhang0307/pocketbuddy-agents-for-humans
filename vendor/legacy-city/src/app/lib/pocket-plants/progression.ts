...
/** A confirmed photo identification contributes one usable card/seed. */
export function grantRecognizedPocketSeed(assetId: string): PocketPlantProgress {
  const normalizedId = normalizePocketPlantAssetId(assetId);
  const current = readPocketPlantProgress();
  const next = {
    ...current,
    inventory: {
      ...current.inventory,
      [normalizedId]: Math.max(0, current.inventory[normalizedId] ?? 0) + 1,
    },
  };
  persist(next);
  emit();
