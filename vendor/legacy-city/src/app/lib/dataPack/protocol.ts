...
function validatePlayback(value: unknown, label: string) {
  ...
  if (!['oss', 'youtube', 'external', 'none'].includes(String(v.provider))) throw new DataPackError('schema', `${label}.provider 无效`);
  const url = string(v.url, `${label}.url`, 2000, true);
  if (url && !/^https:\/\//.test(url)) throw new DataPackError('schema', `${label}.url 必须是 HTTPS 地址`);
  if (v.provider !== 'none' && !url && !v.sourceId) throw new DataPackError('schema', `${label} 缺少可解析的 url 或 sourceId`);
  if (v.sourceUrl !== undefined) {
    const sourceUrl = string(v.sourceUrl, `${label}.sourceUrl`, 2000, true);
    if (sourceUrl && !/^https:\/\//.test(sourceUrl)) throw new DataPackError('schema', `${label}.sourceUrl 必须是 HTTPS 地址`);
  }
  if (v.sourceId !== undefined) string(v.sourceId, `${label}.sourceId`, 300, true);
}
