...
更新：2026-08-28。以下区分已实现、已验证与尚未实机验收。当前真机已安装并启动构建 2026082832；未上传 TestFlight。用户截图已证明此前硬件语音进入 Frost 并打开地图，但当时路线失败；新版实体路线和锁屏导航仍待验收。

21:15 追加：已支持目的地与里程同时生效，并在真实高德验证 3/4/5 公里产生 3.089/3.981/5.017 公里的不同道路。软件完整指令直接规划、硬件来源可请求自动导航、改成四公里保留目的地。详见 [目的地与里程验证记录](FROST-DESTINATION-DISTANCE-2026-08-28.md)。

## 20:39 追加：修复“去西湖的跑步路线”被误当成起点

用户截图中的实际 ASR 是「请帮我规划一下去西湖的跑步路线」。旧的回归仅覆盖「帮我规划下去西湖的路线」，漏掉了“的跑步路线”同时匹配起点模板的情况。因此旧版既提取到目的地“西湖”，又错误提取起点“去西湖”，进入地图后查询起点失败。这次修改不是只跳 Tab 或改回复。

- 目的地表达优先：去/到/跑到西湖的跑步路线 → 当前位置到西湖，单程；只补默认偏好风景好、少路口。只有没有目标时才默认 3 公里环线。
- 同一句含明确的“从杭州龙翔桥出发 / 从杭州龙翔桥到西湖”时保留指定起点；“西湖的跑步路线”仍表示在西湖附近跑。真实后续对话已选择的起点保留，明确改为当前位置时清除。
- 即使 Qwen 给“去西湖”或“西湖”附上原句 evidence，也不能把目的地再次当成起点。Qwen 提示和本地合并校验同时修正。
- 回复直接描述实际目标、形状、偏好，移除无条件声称“3 公里环线”的矛盾句。硬件来源仍直接创建会话并请求自动启动，软件来源仍按缺失条件追问。

### 本次验证

- 新测试在修改前复现了 11 个失败；修复后相关三个文件 101 项通过。全量 225 个文件通过、3 个跳过；2,775 项通过、16 项跳过。TypeScript 与识鸟源码检查通过。
- 完整测试链：截图原句 → 对话解析 → Taskmaster 序列化 → 当前 GPS → 坐标转换 → 西湖 POI → 高德步行结果 → ready、道路折线与转弯。此项使用明确标注的 GPS/高德测试数据，不是实机定位证据。拒绝定位时保持 failed/空路线，不调用步行服务、不使用样例冒充结果。
- 真实浏览器：先输入截图原句，再在软件补充「从杭州龙翔桥出发，风景好、少路口」，经真实 Qwen（页面 Trace 为 qwen3.7-max）自动切换 Earth，真实高德得到 0.57 km 单程道路折线，实跑 0.00 km。没有冒充手机 GPS、硬件录音或启动真实导航。
- 页面证据：`evidence/voice-destination-fix-20260828/`。RouteSession 为 `run-route:mtcxphyg:daaf5e72-5894-48c8-8726-9ffd5969421c`，Task 89。

### 新版手机安装

按统一 archive.mjs 从当前完整源码重建，构建号 2026082831；本机历史与手机原版本 2026082830 已核对，未登录 Apple 后台核对或上传。主应用与两个子应用全量准备，使用全新 DerivedData。

归档与安装前检查均通过：源码/资源 provenance、线稿技能画布、Frost Skills、识鸟 v3、实际 App 图标、App/Widget 构建号及签名、两份描述文件对连接手机的授权。另检查实际打包 JS 含新语音回复且不含旧的矛盾默认句。

20:37 覆盖安装成功，随后从手机回读 2026082831；20:38:55 成功启动，20:39 再次回读版本。没有卸载或清除用户数据。安装后的首次启动命令参数顺序有误，已单独纠正并成功启动；没有因此重复安装。完整回执在 `/Volumes/PocketBuddy-iOS-Dev/Current/Releases/2026082831-FWSHHn/`。

- Source SHA-256：`4a58d931149076db76ed64af14c880a09c999d89f7e4d9ceb68147a51a79f31c`
- Assets SHA-256：`48264dce6a7c3150403075304e43fbd4c99ef5f869e153ad691971bed1adf74f`

已请用户保持手机亮屏、连接硬件后重说原句；尚未收到新版结果。锁屏后新发起规划仍未实现，本次没有删除原生前台保护，也没有宣称锁屏/蓝牙/扬声器已实测通过。之前已启动导航的原生后台 GPS/播报能力与“锁屏新建”是不同验收项。
