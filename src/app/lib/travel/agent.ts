...
export async function runPlan(input: PlanInput, onPhase?: OnTravelPhase): Promise<TripPlan | null> {
  ...
  // ② Travel LoRA 只把自然语言解析成固定协议；LoRA 格式失败时由本机 Qwen 基座做宽松语义理解。
  // 两者都失败就终止本次规划；模型不直接生成景点事实。
  ...
  ph('需求理解通过', planner.source === 'qwen-lora' ? 'Travel LoRA' : 'Qwen Base');
...
// A 线（截图提炼）编排：端侧 vision 读票据 → 端侧脱敏 → 端侧 Qwen/MNN 结构化 → TripArchive 草稿。
