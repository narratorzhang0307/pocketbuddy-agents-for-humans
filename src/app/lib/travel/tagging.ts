// 协作层（A 线）：脱敏文本（多张截图）→ 端侧 Qwen/MNN JSON → TripArchive 草稿。
...
// 多张脱敏文本 → 一次旅程的结构化草稿。端侧未返回合法结构 → null（上层交回手填，不静默上云）。
export async function structureTrip(shots: RawShot[]): Promise<TripArchive | null> {
  ...
  try {
    const response = await runEdgeChatEvidence(prompt, { system, json: true, maxTokens: 760 });
    const o = response.backend === 'mnn' ? extractJSON<Record<string, unknown>>(response.text || '') : null;
    if (!o) return null;
