...
export interface TravelPlannerTrace {
  ...
  source: 'qwen-lora' | 'qwen-base' | 'rules';
  gate: 'parsed' | 'repaired' | 'semantic' | 'fallback';
...
// ── A 线（截图自动提炼，P1）：端侧 vision 读票据 → 端侧脱敏文本 → 端侧 Qwen/MNN 结构化 → TripArchive 多点钉 ──
