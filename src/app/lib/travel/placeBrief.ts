...
export async function loadTravelPlaceBrief(city: string, place: string): Promise<TravelPlaceBrief> {
  ...
  try {
    ...
    // Fetching public sources is a deterministic network tool. Their summary
    // still runs through local Qwen/MNN; this optional Detail view must not
    // silently turn a Travel Skill run into cloud inference.
    const result = await runQwenGroundedPlaceBrief(prompt, PLACE_BRIEF_SYSTEM);
    ...
    if (result.backend !== 'mnn' || !isGroundedPlaceBrief(text, sources)) {
