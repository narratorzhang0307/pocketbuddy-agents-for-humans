import { runQwenAdapter, runQwenBase } from '../../../../frost-agent/edge/httpQwenEdge';
...
export const BASE_TRAVEL_SEMANTIC_PROMPT = '你是旅行需求抽取器。只允许输出一行固定模板：目的地=；天数=；兴趣=；节奏=；绕路限制=；每日站数=。只抄用户已给出的值，未提及写“无”。禁止出现模板外的任何文字，禁止补充景点、酒店、预算或行程。';
...
function repeatedOutput(text: string): boolean {
  const compact = text.replace(/\s/g, '');
  if (compact.length < 12) return false;
  const counts = new Map<string, number>();
  for (const char of compact) counts.set(char, (counts.get(char) || 0) + 1);
  return Math.max(...counts.values()) / compact.length > 0.55 || /(.{1,8})\1{5,}/u.test(compact);
}
...
function semanticPlannerObject(text: string, context: PlannerIntentContext): Record<string, unknown> | null {
  const output = text.replace(/<think>[\s\S]*?<\/think>/gi, '').trim();
  if (output.length < 4 || repeatedOutput(output)) return null;
  const interests = interestsToPreferences([output]);
  const pace = localPace(output);
  const crowd = localCrowd(output);
  const mix = localMix(output);
  const walking = explicitWalkingLimit(output);
  const compact = localCompactRoute(output);
  const avoid = localAvoidTerms(output);
  const mustVisit = localMustVisitTerms(output);
  const dietary = localDietary(output);
  const mentionsDestination = output.includes(context.destination);
  const mentionsDuration = new RegExp(`${context.days}\\s*(?:天|日)`).test(output);
  const extracted = interests.length || pace || crowd || mix || walking || compact || avoid.length || mustVisit.length || dietary.length;
  if (!mentionsDestination || (!mentionsDuration && !extracted)) return null;
  return {
    destination: context.destination,
    duration_days: context.days,
    preferences: interests,
    constraints: { walking_limit_km: walking, compact_route: compact, avoid, must_visit: mustVisit, dietary },
    soft_constraints: { pace, crowd_tolerance: crowd, mix_strategy: mix },
...
export async function inferPlannerIntent(context: PlannerIntentContext): Promise<TravelPlannerTrace> {
  ...
  let response = await runQwenAdapter(prompt, {
    system: TRAINED_TRAVEL_SYSTEM_PROMPT,
    json: true,
    adapter: 'travel-planner',
    maxTokens: 384,
  });
  let parsed = response.backend === 'mnn' && response.text
    ? parsePlannerModelObject(response.text)
    : { value: null, repaired: false };
  const loraError = response.backend !== 'mnn' || !response.text
    ? response.error || 'mnn_runtime_unavailable'
    : !parsed.value
      ? 'invalid_model_json'
      : !hasPlannerSemantics(parsed.value)
        ? 'invalid_model_contract'
        : '';
  let source: TravelPlannerTrace['source'] = 'qwen-lora';

  // LoRA 格式失败时，让同机 Qwen Base 用自然语言复述语义，不再用严格 JSON 门禁考基座。
  // 两个端侧模型都失败时直接失败闭合，不产出一份纯规则行程。
  if (loraError) {
    source = 'qwen-base';
    response = await runQwenBase(prompt, {
      system: BASE_TRAVEL_SEMANTIC_PROMPT,
      json: false,
      maxTokens: 64,
    });
    parsed = response.backend === 'mnn' && response.text
      ? { value: semanticPlannerObject(response.text, context), repaired: false }
      : { value: null, repaired: false };
    const baseError = response.backend !== 'mnn' || !response.text
      ? response.error || 'mnn_runtime_unavailable'
      : !parsed.value
        ? 'invalid_model_semantics'
        : '';
    if (baseError) throw new Error(`qwen_intent_failed:lora:${loraError};base:${baseError}`);
  }

  if (!parsed.value) throw new Error('qwen_intent_failed:invalid_model_output');
  ...
  return {
    ...fallback,
    source,
    gate: source === 'qwen-base' ? 'semantic' : parsed.repaired ? 'repaired' : 'parsed',
    interests: [...new Set([...context.preferences, ...interestsToPreferences(interestValues)])],
    walkingLimitKm: modelWalking || localWalkingLimit,
    compactRoute: Boolean(modelWalking || hard.compact_route === true || localCompactRoute(context.requestText)),
    ...
    inferenceMs: response.elapsedMs,
    adapterLoaded: source === 'qwen-lora' ? response.adapterLoaded === true : false,
    runtimeModel: response.model || 'Qwen3-VL-2B / MNN native',
    raw: response.text,
  };
}
