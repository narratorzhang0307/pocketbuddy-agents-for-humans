...
export function chooseArtifactInscriptionCandidate(baseRaw: string, loraRaw: string): ArtifactInscriptionResult {
  ...
  if (!loraIssue && baseIssue) {
    return { ...common, rawText: loraCandidate, confidence: 0.66, source: 'rubbing-lora', needsConfirmation: false, gateReason: `采用 LoRA 候选：${baseIssue}` };
  }
  if (loraIssue && !baseIssue) {
    return { ...common, rawText: baseCandidate, confidence: 0.58, source: 'base-fallback', needsConfirmation: false, gateReason: `已采用 Base 候选：${loraIssue}` };
  }
