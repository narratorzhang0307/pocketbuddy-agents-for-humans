...
export function isRunRouteRequest(text: string): boolean {
  return /跑步路线|跑步导航|规划.{0,40}(?:跑|慢跑)|(?:带我跑|跑到|跑去|开始跑步)/.test(text)
    && !/^(?:不要|别|取消|停止)/.test(text.trim()) && !/跑完|跑步记录/.test(text);
}
...
const PREFERENCES: Array<[RunRoutePreference, RegExp]> = [
  ...
  ['low_crossings', /少(?:点)?(?:红绿灯|路口|转弯)|路口少|红绿灯少/],
...
export function parseRunRouteFields(text: string): Partial<RunRouteDraft> & { current_location?: boolean } {
  ...
  if (destination) fields.goal = { type: 'destination', query: destination[1].replace(/(?:的)?(?:跑步)?路线.*$|[，,].*$/, '').trim() };
  const area = text.match(/(?:在|围绕|绕着|从)\s*([^，。！？\n]{2,30}?)(?:附近|周边|出发|开始|跑|慢跑)/)
    || text.match(/(?:规划|安排)(?:一下|下)?\s*([^，。！？\n]{2,30}?)的跑步路线/);
...
export async function advanceRunRouteDialogue(text: string, previous?: RunRouteDraft, voice = false, signal?: AbortSignal): Promise<RunRouteDialogue> {
  ...
  return { draft, reply: `${voice ? '语音快捷规划：' : '条件齐了：'}${describeRunRoute(input)}。正在交给高德计算真实道路并打开中间的行动地图。${voice ? '未说的条件采用 3 公里环线、风景好和少路口；定位就绪后尝试开始导航。' : '路线出来后，点“开始沿线跑”启用定位和导航。'}`, choices: [], needsInput: false, input, parser };
}
