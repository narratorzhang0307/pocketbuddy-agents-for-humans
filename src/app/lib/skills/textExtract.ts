...
import { extractJSON } from './enrichEntity';
...
/**
 * 文本→结构化字段。默认只跑端侧 Qwen/MNN（JSON 模式），不在公共 Skill 中静默回退云端。
 * 单字段兜底：端侧没出 JSON、而 schema 只有一个字段时，文本首行（冒号后）即答案——避免白丢结果。
 */
