...
//     本 Skill 的【① 视觉读图】只调端侧 Qwen3-VL/MNN，
...
//   + [textExtract]（② 文字→字段，默认仅端侧）。两半截各自独立、对称：
//     visionRead    : 图  → 文字
//     textExtract   : 文字 → 字段
//     visionExtract : 图  → 字段
...
export interface VisionExtractResult {
  ...
  structuredVia: 'edge' | 'cloud' | 'none';  // cloud 仅保留历史记录兼容；本公共入口不自动上云
...
export async function visionExtract(input: VisionExtractInput): Promise<VisionExtractResult> {
  ...
  // ② 结构化：脱敏文本交 [textExtract]（默认仅端侧；原图全程不参与第②步）。
