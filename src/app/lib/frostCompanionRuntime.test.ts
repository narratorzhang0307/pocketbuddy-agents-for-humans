...
describe('badge inputs use the main Frost runtime', () => {
  ...
  it.each(['帮我规划下去西湖的路线', '帮我规划下去西湖的跑步路线', '请帮我规划一下去西湖的跑步路线'])(
    ...
    expect(result.session.status).not.toBe('waiting_user');
    const route = readRunRouteSession(getActiveRunRouteSessionId()!);
    expect(route?.input).toMatchObject({ start: 'current_location', goal: { type: 'destination', query: '西湖' }, shape: 'one_way', auto_start: true });
    expect(route?.input.start_query).toBeUndefined();
    const open = vi.fn(); const navigate = createFrostAutoNavigation({ isActive: () => true, open });
    expect(await navigate({ result, input: { text, origin } })).toBe(true);
    expect(open).not.toHaveBeenCalled();
    expect(route?.actual_track).toEqual([]); // Text input is not proof of physical ASR, GPS or background execution.
  });

  it.each(['phone', 'badge_voice'] as const)('opens the returned route on Earth from %s even after the creation notification was missed', async channel => {
    const text = '帮我规划杭州西湖白堤的跑步路线，三公里，环线，风景好、少路口';
    const origin = channel === 'phone' ? { channel } : { channel, inputId: 'badge:test:route:handoff' };
    const result = await sendFrostAgentMessage(text, origin);
    const id = getActiveRunRouteSessionId()!;
    const open = vi.fn(), routeOpened = vi.fn();
    const release = subscribeRunRouteOpen(routeOpened);
    routeOpened.mockClear();
    const navigate = createFrostAutoNavigation({ isActive: () => true, open });
    try {
      expect(await navigate({ result })).toBe(false); // Restored history must not steal the page.
      expect(await navigate({ result, input: { text, origin } })).toBe(true);
      expect(await navigate({ result, input: { text, origin } })).toBe(false);
      expect(routeOpened).toHaveBeenCalledExactlyOnceWith(id);
      expect(open).not.toHaveBeenCalled(); // Do not reopen the blank Skill form.
      expect(readRunRouteSession(id)).toMatchObject({ input: { goal: { distance_m: 3000 } }, actual_track: [] });
    } finally { release(); }
  });
