...
export default function SkillAvatar({ skillId, size = 52, className = '' }: {
  ...
  return <span className={`block shrink-0 overflow-hidden rounded-xl ${className}`} style={{ width: size, height: size }}>
    <img src={avatar.src} alt={`${avatar.name}头像`} width={size} height={size}
