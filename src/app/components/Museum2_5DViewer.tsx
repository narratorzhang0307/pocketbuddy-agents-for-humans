...
export default function Museum2_5DViewer({
  ...
  const selectedDemo = runtimeDemo || MUSEUM_2_5D_DEMOS.find((demo) => demo.id === demoId) || MUSEUM_2_5D_DEMOS[0];
  ...
  useEffect(() => {
    if (runtimeDemo) return;
    const next = museum2_5DDemoFromUrl(assetUrl);
    setDemoId(next.id);
  }, [assetUrl, runtimeDemo]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    return () => { cancelled = true; };
  }, [runtimeDemo, selectedDemo.manifestUrl]);
      ...
      <div className="absolute inset-x-0 top-0 z-10 flex items-center justify-between border-b border-black/20 bg-white/75 px-3 py-2 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className={`border border-black px-1.5 py-0.5 font-pixel text-[6px] ${assetQualityGate === 'reject' ? 'bg-[#ff8b7c]' : assetQualityGate === 'checking' ? 'bg-[#ffd166]' : 'bg-[#7CFF6B]'}`}>
            {assetQualityGate === 'pass' ? '抠图自检通过' : assetQualityGate === 'legacy' ? 'MNN FP16' : assetQualityGate === 'reject' ? '展示静态帧' : '自检中'}
          </span>
        </div>
      </div>
      {!hideDemoSwitch && !runtimeDemo && <div className="absolute left-2 right-2 top-10 z-30 flex gap-1 overflow-x-auto pb-1" aria-label="切换 2.5D 展品示例">
