...
export default function FrostVoiceControls({ reply, connected, recording, working, run }: Props) {
  ...
  return <div className="space-y-2 border-t border-black/20 pt-2" aria-label="Frost MiniMax 语音">
    ...
    <div className="flex flex-wrap gap-2">
      <button className={button} disabled={working || !reply} onClick={() => setText([...(reply || '').trim()].slice(0, FROST_VOICE_MAX_TEXT).join(''))}>填入最近回复</button>
      <button className={button} disabled={working || !count || cached} onClick={() => void run(generate)}>{generating ? '生成中…' : cached ? '已生成' : '生成语音（可能计费）'}</button>
    </div>
    <details>
      <summary>语音访问设置</summary>
      <p>请填访问码，勿填 API Key。</p>
