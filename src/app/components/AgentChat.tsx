    ...
    try {
      ...
      if (!reply) {
        ...
        if (backend === 'edge') {
          ...
          if (!reply) reply = '端侧 Qwen 本次没有返回有效内容。请确认 Qwen Base / MNN 已就绪后重试；不会自动把文字上传到云端。';
        } else {
          ...
          try {
            ...
            if (!reply) reply = failed ? '云端增强暂时不可用。可切回「端侧」继续离线使用。' : '我这边没生成出内容，换个说法再试试。';
          }
        }
      }
  ...
  return (
    <div className="h-full flex flex-col bg-[#EAEAEA] min-h-0">
      <div className="shrink-0 flex items-center justify-between border-b-2 border-black bg-white px-3 py-2">
        <div>
          <div className="font-pixel text-[7px] tracking-widest">QWEN 执行位置</div>
          <div className="mt-0.5 text-[9px] text-black/45">端侧不上传文字；云端由你主动开启</div>
        </div>
        <div className="flex border-2 border-black bg-[#EAEAEA] p-0.5">
          <button type="button" disabled={busy} onClick={() => setBackend('edge')} className={`px-2 py-1 text-[10px] font-bold ${backend === 'edge' ? 'bg-black text-[#7CFF6B]' : 'text-black/45'}`}>端侧</button>
          <button type="button" disabled={busy} onClick={() => setBackend('cloud')} className={`px-2 py-1 text-[10px] font-bold ${backend === 'cloud' ? 'bg-black text-[#7CFF6B]' : 'text-black/45'}`}>云端增强</button>
        </div>
      </div>
      <div className="flex-1 overflow-y-auto px-3 py-3 flex flex-col gap-3 min-h-0">
        ...
        {turns.map((t, i) => t.role === 'user' ? (
          ...
          <div key={i} className="flex items-start gap-2 max-w-[94%]">
            ...
            <div className="flex flex-col gap-1 min-w-0">
              ...
              {t.execution && !t.error && <div className="text-[9px] text-black/45">{t.execution.backend === 'mnn' ? 'MNN 端侧' : t.execution.backend === 'qwen-cloud' ? 'Qwen 云端增强' : t.execution.backend}{t.execution.model ? ` · ${t.execution.model}` : ''}{typeof t.execution.elapsedMs === 'number' ? ` · ${(t.execution.elapsedMs / 1000).toFixed(1)}s` : ''}</div>}
        ...
        {busy && <div className="font-pixel text-[8px] text-black/45 tracking-widest animate-pulse">⋯ {backend === 'edge' ? 'QWEN / MNN 端侧运行' : 'QWEN 云端增强'} ⋯</div>}
        <div ref={endRef} />
      </div>
