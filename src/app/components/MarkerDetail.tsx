...
export interface MarkerDetailData {
  ...
  // music
  artist?: string; songIntro?: string; artistIntro?: string; audioUrl?: string; sourceUrl?: string; model?: string; durationSec?: number;
...
const spatialTypeLabel: Record<MarkerDetailData['kind'], string> = {
  photo: 'PHOTO', movie: 'MOVIE', book: 'BOOK', mapping: 'MAPPING', travel: 'TRAVEL',
  music: 'MUSIC', council: 'COUNCIL', exhibition: 'EXHIBITION', museum: 'MUSEUM', custom: 'CUSTOM',
};

const spatialSourceLabel = (data: MarkerDetailData) => {
  if (data.kind === 'mapping') return data.sourceRef || data.packName || 'PRIVATE DATA PACK';
  if (data.kind === 'photo') return 'EXIF / MEDIASTORE';
  if (data.kind === 'book') return 'BOOKS-SKILL / DATA PACK';
  if (data.kind === 'movie') return 'MOVIES-SKILL / DATA PACK';
  if (data.kind === 'music') return data.sourceUrl ? 'MUSIC SOURCE URL' : 'MUSIC-SKILL / DATA PACK';
  if (data.kind === 'travel') return 'TRAVEL / LOCAL RECORD';
  if (data.kind === 'exhibition' || data.kind === 'museum') return 'EXHIBITION / LOCAL RECORD';
  return data.agentName || 'LOCAL RECORD';
};
...
function detailTags(d: MarkerDetailData): RelatedTag[] {
  ...
  push('导演', d.director); push('作者', d.author); push('歌手', d.artist); push('类型', d.genre); push('流派', d.movement);
...
export default function MarkerDetail({ data, onClose, onRemove, onView3D, onSelectRelated, transparentBackdrop = false }: { data: MarkerDetailData; onClose: () => void; onRemove?: (id: string) => void; onView3D?: (url: string, format: string) => void; onSelectRelated?: (r: RelatedItem) => void; transparentBackdrop?: boolean }) {
  useEffect(() => {
  ...
  return (
    <motion.div
      ...
      data-backdrop={transparentBackdrop ? 'clear' : 'dimmed'}
      className={`absolute inset-0 z-[120] flex items-center justify-center p-6 ${transparentBackdrop ? 'bg-transparent' : 'bg-black/70 backdrop-blur-sm'}`}
      onClick={onClose}
    >
      <motion.div
        initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
        data-card-shadow={transparentBackdrop ? 'none' : 'offset'}
        className={`w-[300px] max-w-full bg-white border-[3px] border-black relative ${transparentBackdrop ? '' : 'shadow-[6px_6px_0_#000]'}`}
      ...
      >
          ...
          <div>
            ...
            <div className="px-3 py-2.5">
              ...
              <div className="flex items-center gap-1.5 mt-2.5">
                ...
                <span className="font-pixel text-[7px] text-black/50 tracking-wider">
                  钉于 {data.geoKind === 'story' ? '故事地' : data.geoKind === 'author' ? '作者地' : data.geoKind === 'reading' ? '阅读地' : ''}{data.place || '故事之地'}{data.date ? ` · 读于 ${data.date}` : ''}
                </span>
              </div>
            </div>
          </div>
        ...
        {data.kind === 'music' && (
          <div>
            <div className="flex items-center justify-between gap-2 px-2.5 py-1.5 bg-black">
              <span className="font-pixel text-[7px] tracking-widest text-[#00ff88]">MUSIC · 地球唱片</span>
              {data.model && <span className="font-pixel text-[5px] text-[#00ff88]/65 truncate">{data.model}</span>}
            </div>
            <div className="px-3 py-3">
              <div className="text-[16px] font-bold leading-tight">{data.title || data.city}</div>
              {data.artist && <div className="mt-1 text-[11px] text-black/60">{data.artist}{data.year ? ` · ${data.year}` : ''}{data.genre ? ` · ${data.genre}` : ''}</div>}
              {data.audioUrl && <audio controls preload="metadata" src={data.audioUrl} className="mt-2 h-8 w-full" />}
              {data.songIntro && <div className="mt-2 text-[11px] leading-relaxed text-black/75"><span className="font-bold">歌曲：</span>{data.songIntro}</div>}
              {data.artistIntro && <div className="mt-1.5 text-[10px] leading-relaxed text-black/60"><span className="font-bold">歌手：</span>{data.artistIntro}</div>}
              <div className="mt-2.5 flex items-center justify-between gap-2">
                <span className="font-pixel text-[7px] text-black/45">● {data.geoKind === 'song_city' ? '歌曲城市' : '歌手城市'} · {data.place || data.city || '已钉地球'}</span>
                {data.sourceUrl && <a href={data.sourceUrl} target="_blank" rel="noopener noreferrer" className="font-pixel text-[6px] underline text-black/45">SOURCE</a>}
              </div>
            </div>
          </div>
        )}
        ...
        {/* 同一种空间对象：真实落点统一露出类型、坐标、日期与来源。只在地球点击到有坐标的标记时出现。 */}
        {Number.isFinite(data.lat) && Number.isFinite(data.lng) && (
          <div className="border-t-2 border-black bg-[#F5F2E9] px-3 py-2" data-testid="spatial-object-fields">
            <div className="flex items-center justify-between">
              <span className="font-pixel text-[7px] tracking-widest text-[#7A4DD6]">SPATIAL OBJECT · WGS84</span>
              <span className="font-pixel text-[6px] text-black/40">LOCAL-FIRST INDEX</span>
            </div>
            <div className="mt-1.5 grid grid-cols-2 gap-x-2 gap-y-1">
              <div className="border-l-[3px] border-[#C6923A] pl-1.5">
                <div className="font-pixel text-[6px] text-black/35">TYPE</div>
                <div className="font-pixel text-[7px] text-black/70">{spatialTypeLabel[data.kind]}</div>
              </div>
              <div className="border-l-[3px] border-[#8F63D3] pl-1.5">
                <div className="font-pixel text-[6px] text-black/35">COORD</div>
                <div className="font-pixel text-[7px] text-black/70">{data.lng!.toFixed(5)}, {data.lat!.toFixed(5)}</div>
              </div>
              <div className="border-l-[3px] border-[#EC684B] pl-1.5">
                <div className="font-pixel text-[6px] text-black/35">DATE</div>
                <div className="font-pixel text-[7px] text-black/70">{data.date || data.createdAt?.slice(0, 10) || '—'}</div>
              </div>
              <div className="border-l-[3px] border-[#3E9E9A] pl-1.5 min-w-0">
                <div className="font-pixel text-[6px] text-black/35">SOURCE</div>
                <div className="font-pixel text-[7px] text-black/70 truncate">{spatialSourceLabel(data)}</div>
              </div>
            </div>
          </div>
        )}
