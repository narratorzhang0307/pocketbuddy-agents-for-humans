...
import {
  ArrowLeft, ArrowRight, Camera, Check, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, Database, HeartPulse,
  GripVertical, MapPin, Play, Plus, RotateCcw, Save, ShieldCheck, Sparkles,
  Volume2, WandSparkles, X,
} from 'lucide-react';
import {
  CAPABILITY_DEFINITIONS, compileSkillDraft, getCanvasSkill, saveCanvasSkill,
    ...
    <div className="mx-auto mt-2 flex h-[560px] w-full max-w-[342px] flex-col overflow-hidden rounded-[28px] border-[3px] border-[#26231f] bg-[#f8f1e3]">
      ...
      {!flipped ? <div className="flex min-h-0 flex-1 flex-col p-3">
        ...
        <section className="rounded-[18px] border-2 border-[#26231f] bg-[#191a17] p-3 text-[#f8f1e3]">
          <div className="flex items-center justify-between gap-2"><small className="font-pixel text-[5px] text-[#7CFF6B]">能力契约 · CONTRACT</small><span className="rounded-full border border-[#7CFF6B] px-2 py-0.5 font-pixel text-[4px] text-[#7CFF6B]">能力声明</span></div>
      ...
      {stage === 'sketch' && <>
        ...
        <section className="px-3 pb-4 pt-3">
          ...
          <div className="mt-3 rounded-[14px] border-2 border-black bg-[#f8f1e3] p-2.5">
            <div className="flex items-center justify-between"><span><b className="block text-[9px]">技能图构建检查</b><small className="mt-0.5 block text-[7px] text-black/45">结构化前先满足最小结构要求</small></span><Sparkles className="h-4 w-4" /></div>
          ...
          {compileAttempted && compileResult.issues.length > 0 && <div ref={compileIssuesRef} role="alert" className="mt-2 rounded-[14px] border-2 border-[#b3261e] bg-[#fff0ed] p-2.5">
            ...
            {compileResult.issues.map((issue, index) => <div key={`${issue.code}-${issue.node_id || index}`} className="mt-2 border-t border-[#b3261e]/20 pt-2 first:mt-1 first:border-0 first:pt-0">
              ...
              {issue.code === 'missing_outcome' && <div className="mt-1.5 flex flex-wrap gap-1.5"><button type="button" onClick={() => addBlock('action.voice')} className="rounded-full border-2 border-black bg-white px-2.5 py-1 text-[7px] font-black">添加语音通知</button><button type="button" onClick={() => addBlock('store.local')} className="rounded-full border-2 border-black bg-[#ffd34e] px-2.5 py-1 text-[7px] font-black">添加完成与证据</button></div>}
            </div>)}
          </div>}
        </section>
