...
export default function FitnessAgentEntry({ onOpen }: { onOpen: () => void }) {
  return (
    <button
      type="button"
      aria-label="打开 Fitness Agent 总路由"
    ...
    >
      <span className="grid h-[52px] w-[52px] place-items-center overflow-hidden border-2 border-black bg-[#fffaf0]">
        <img src={FROST_AVATAR.src} alt="Agent 世界焦糖腊肠犬" className="h-full w-full object-cover" draggable={false} />
      </span>
      <span className="min-w-0">
        <span className="block font-pixel text-[11px] tracking-wider text-black">FITNESS AGENT</span>
        <span className="mt-0.5 block text-[10.5px] leading-snug text-black/60">总路由 · 理解饮食、训练与恢复目标，调度已装备的 Skills 与子 Agent。</span>
